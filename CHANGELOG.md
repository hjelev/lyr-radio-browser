...
- Fixed the elapsed-time display showing an absurd value (~495 000 hours, the Unix epoch) on playing stations: the station bitrate was passed to LMS in bps where kbps is expected (LMS multiplies by 1000 itself), and stations with an unknown bitrate got none at all — which sent LMS's elapsed-time estimation down a broken fallback path on live radio. The bitrate is now always passed in kbps, defaulting to 128 when Radio Browser doesn't report one.
...
- Fixed a regression from the fix above that broke playback of every station: the direct stream URL was tagged with a `#rb-<uuid>` fragment to recover the uuid for click tracking, but LMS parses the connection host straight out of the full URL including the fragment, so every stream failed to open. The uuid is now recovered from a `stream URL -> uuid` cache entry recorded when the station was listed, leaving the play URL untouched.
